...
//printf("Free Memory:%luMB = %luKB,\tTotal Memory:%luMB = %luKB\n",freeMem/1024/1024,freeMem/1024,totalMem/1024/1024,totalMem/1024);
        ...
        if(i>0){
            if(foveatedmatching==0){
                ...
                if(i>=foveatelevel){disp[i-1]=subsampleDisp(disp[i], scalefactor, 3, width[i], height[i], width[i-1], height[i-1]);
}
                else{disp[i-1]=foveatedsubsampleDisp(disp[i], scalefactor, 3, width[i], height[i], width2, height2);
